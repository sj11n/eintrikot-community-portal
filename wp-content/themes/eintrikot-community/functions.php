...
add_action('wp_enqueue_scripts', function () {
    if (is_page_template('mvp-public') || is_page_template('mvp-portal') || is_singular('post')) {
        ...
        wp_enqueue_style('eintrikot-mvp', get_theme_file_uri('assets/mvp.css'), array(), '0.5.1');
        wp_enqueue_style('eintrikot-wp-adapter', get_theme_file_uri('assets/wordpress.css'), array('eintrikot-mvp'), '0.5.1');
    }
}, 30);

add_action('login_enqueue_scripts',function(){wp_enqueue_style('eintrikot-login',get_theme_file_uri('assets/login.css'),array(),'0.5.1');});
